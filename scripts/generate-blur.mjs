...
// To handle the import of TS in pure Node without tsx, we can read the file and parse it,
// but let's try the user's suggestion first.
...
async function main() { 
  ...
  for (const project of projects) { 
    ...
    for (const img of project.images) { 
      if (!img) continue; 
      const blur = await getBlurDataURL(img); 
      result[project.slug].push({ src: img, blur }); 
      console.log(`✓ ${img}`); 
    } 
  } 
  await fs.writeFile( 
